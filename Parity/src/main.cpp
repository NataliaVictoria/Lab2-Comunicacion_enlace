...
void loop()
{
  if (tx_counter <= MESSAGE_NUM)
  {
    ...
    if (TX)
    {
      ...
      String error = BitParity(msg, MODE);
...
String BitParity(String msg, int mode)
{
  ...
  else if (mode == 2)
  {
    ...
    error_count = random(0, msg.length()*8);
  }
  int mem[error_count];
  bool repeated = false;
...
void probabilidad(){
    ...
    if(MODE==0){
      global::oled->println("Con un bit de error");
    }
    else if(MODE == 1){
      global::oled->println("Con 2 bits de error");
    }
    else if(MODE == 2){
      global::oled->println("Con 3 bits de error");
    }
    else{
